...
from network import C3D_model, I3D #R2Plus1D_model, R3D_model
...
#    ctypes.cdll.LoadLibrary('caffe2_nvrtc.dll')
#  File "/Users/deemaazizn/opt/anaconda3/lib/python3.8/ctypes/__init__.py", line 459, in LoadLibrary
#    return self._dlltype(name)
#  File "/Users/deemaazizn/opt/anaconda3/lib/python3.8/ctypes/__init__.py", line 381, in __init__
#    self._handle = _dlopen(self._name, mode)
#OSError: dlopen(caffe2_nvrtc.dll, 6): image not found

# Use GPU if available else revert to CPU
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
print("Device being used:", device)

nEpochs = 10  # Number of epochs for training was 100
...
def train_model(dataset=dataset, save_dir=save_dir, num_classes=num_classes, lr=lr,
    ...
    if modelName == 'C3D':
        ...
        num_ftrs = model.fc.in_features
        model.fc = nn.Linear(num_ftrs, 2)# Deema: I guess they added a linear layer to reduce output to 2
        model = model.to(device)
        train_params = model.parameters()
#    elif modelName == 'MC3':
        # model = R3D_model.R3DClassifier(num_classes=num_classes, layer_sizes=(2, 2, 2, 2))
...
#        train_params = model.parameters()
    ...
    train_dataloader = DataLoader(VideoDataset(dataset=dataset, split='train',clip_len=16, preprocess=False),  batch_size= 8, shuffle=True, num_workers=4) # preprocess was False
